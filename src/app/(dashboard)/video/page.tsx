...
export default function VideoPage() {
    ...
    const startNewMeeting = (openInNewWindow = false) => {
        ...
        if (openInNewWindow) {
            // Open in popup window so it persists when navigating
            window.open(
                url,
                'CollabFlow Meeting',
    ...
    const joinMeeting = (openInNewWindow = false) => {
        if (meetingId.trim()) {
            const url = `/video/${meetingId.trim()}`;
    ...
    return (
        <div className="flex h-[calc(100vh-3.5rem)] items-center justify-center bg-gradient-to-br from-background via-background to-primary/5 p-4">
            <motion.div
